 ...
 *
 * Env vars:
 *   GH_USERNAME  - GitHub login to fetch contributions for.
 *   GH_TOKEN     - token with access to the GraphQL API.
...
function buildDemoWeeks() {
  ...
  return Array.from({ length: COLS }, (_, col) => ({
    contributionDays: Array.from({ length: ROWS }, (_, row) => {
      const wave = (col * 7 + row * 3 + (col % 5) * 2) % 17;
      const count = wave < 5 ? 0 : Math.min(18, wave + (row % 3));
      const level = count === 0 ? 0 : Math.min(4, Math.ceil(count / 4));
      const date = new Date(today);
      date.setDate(today.getDate() - ((COLS - 1 - col) * 7 + (ROWS - 1 - row)));
...
async function main() {
  const hasGitHubCredentials = Boolean(USERNAME && TOKEN);
  const weeks = hasGitHubCredentials
    ? await fetchWeeks()
    : buildDemoWeeks();

  console.log(
    hasGitHubCredentials
      ? `Buscando contribuições de ${USERNAME}...`
      : "Gerando uma prévia local. O GitHub Actions usará suas contribuições reais.",
  );
